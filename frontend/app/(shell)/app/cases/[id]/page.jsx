...
export default function DetailPage({ params }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      const res = await fetch("/api/cases/" + params.id, { headers: token ? { Authorization: "Bearer " + token } : {} });
  ...
  async function onDelete() {
    ...
    const sess = await supabase.auth.getSession();
    const token = (sess && sess.data && sess.data.session && sess.data.session.access_token) ? sess.data.session.access_token : "";
    const res = await fetch("/api/cases/" + params.id, { method: "DELETE", headers: token ? { Authorization: "Bearer " + token } : {} });
    const json = await res.json().catch(() => null);
  ...
  return (
    <main style={{ minHeight: "calc(100vh - 3.2rem)", padding: "1.6rem 1.4rem" }}>
      <div className="card" style={{ maxWidth: 720 }}>
        <h1 style={{ marginTop: 0 }}>Case detail</h1>
          ...
          <label style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            ...
            />
              ...
              <div style={{ display: "flex", gap: 10, marginTop: 6, flexWrap: "wrap" }}>
                ...
                <button className="btn btn-ghost" type="button" onClick={() => router.push("/app/cases")}>Back</button>
