...
export default function ListPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const res = await fetch("/api/cases", { headers: token ? { Authorization: "Bearer " + token } : {} });
    ...
    <main style={{ minHeight: "calc(100vh - 3.2rem)", padding: "1.6rem 1.4rem", display: "flex", flexDirection: "column", gap: 12 }}>
      <div className="card" style={{ maxWidth: 1100 }}>
        <div style={{ display: "flex", justifyContent: "space-between", gap: 10, alignItems: "center" }}>
          <div>
            <h1 style={{ margin: 0, fontSize: 20 }}>Cases</h1>
            <p className="muted" style={{ margin: "6px 0 0" }}>List, create, and manage your Cases.</p>
          </div>
          <Link className="btn btn-primary" href={"/app/cases/new"} style={{ textDecoration: "none" }}>
            New Case
          </Link>
        </div>
      </div>
      ...
      <div className="card" style={{ maxWidth: 1100 }}>
        {rows.length === 0 ? (
          <div className="muted">No records yet. Create your first Case.</div>
        ) : (
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.id}>
                  ...
                  <td>
                    <Link href={"/app/cases/" + r.id} style={{ textDecoration: "none" }}>View</Link>
                  </td>
                </tr>
              ))}
            </tbody>
