...
export default function NewPage() {
  ...
  async function onSubmit(e) {
    ...
    router.push("/app/issues/" + json.data.id);
  }

  return (
    <main style={{ minHeight: "calc(100vh - 3.2rem)", padding: "1.6rem 1.4rem" }}>
      <div className="card" style={{ maxWidth: 720 }}>
        <h1 style={{ marginTop: 0 }}>New Issue</h1>
        <p className="muted" style={{ marginTop: 6 }}>Create a new Issue record in Supabase.</p>
        ...
        <form onSubmit={onSubmit} style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 14 }}>
          ...
          <div style={{ display: "flex", gap: 10, marginTop: 6 }}>
            ...
            <button className="btn btn-ghost" type="button" onClick={() => router.push("/app/issues")}>Cancel</button>
          </div>
        </form>
      </div>
    </main>
