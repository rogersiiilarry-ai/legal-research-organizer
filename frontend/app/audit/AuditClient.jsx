...
// frontend/app/audit/AuditClient.jsx
...
function resolveAnalysisRow(payload) {
  if (!payload || typeof payload !== "object") return null;
  return payload.analysis || payload.data || payload.row || payload.analysisRow || null;
}

function resolveMeta(analysisRow, payload) {
  const p = payload || {};
  const raw = analysisRow?.meta ?? p?.meta ?? null;
...
function resolveFindings(payload) {
  const p = payload || {};
  const analysis = resolveAnalysisRow(p);
  const meta = resolveMeta(analysis, p);

  const candidates = [
    p.findings,
    p.findings?.rows,
    analysis?.findings,
    analysis?.findings?.rows,
    meta?.findings,
    meta?.findings?.rows,
...
function resolveExportAllowed(payload) {
  const p = payload || {};
  const analysis = resolveAnalysisRow(p);
  const meta = resolveMeta(analysis, p);
...
function formatStatus(analysisRow) {
  const s = safeStr(analysisRow?.status || "").trim();
  return s || "—";
}

function formatSummary(analysisRow) {
 ...
 */
function patchRefreshedWithExecuteFindings(refreshedPayload, executePayload) {
  ...
  const out = refreshedPayload && typeof refreshedPayload === "object" ? { ...refreshedPayload } : {};
...
export default function AuditClient() {
  ...
  const [runningMaterialize, setRunningMaterialize] = useState(false);
  const [runningExecute, setRunningExecute] = useState(false);

  const [tierChoice, setTierChoice] = useState("basic"); // "basic" | "pro"

  function setUiErr(message) {
  ...
  async function fetchAnalysis(id) {
    return await fetchJson(`/api/analyses/${encodeURIComponent(id)}`);
  }
  ...
  useEffect(() => {
    ...
    async function loadDocs() {
      ...
      try {
        const j = await fetchJson(`/api/documents?limit=50`);
  ...
  const shownAnalysisId = useMemo(() => {
    return String(analysisPayload?.analysisId || analysisIdFromQuery || "").trim();
  ...
  async function onMaterialize(e) {
    ...
    try {
      ...
      const newAnalysisId = String(j?.analysisId || j?.analysis_id || "").trim();
  ...
  async function onExecute(e) {
    ...
    const id = String(analysisPayload?.analysisId || analysisIdFromQuery || "").trim();
    ...
    try {
      ...
      const exec = await res.json().catch(() => ({}));
      ...
      if (!res.ok) throw new Error(exec?.error || exec?.message || `Execute failed (${res.status})`);
